...
// Handlebars Helper function for decimals
...
app.get("/divisions", function(req, res) {
    let query1 = `SELECT * FROM Divisions ORDER BY maxAge DESC;`;
...
app.post("/addDivision", function(req, res) {
    ...
    db.pool.query(query1, function(error, rows, fields){
        ...
        if (error) {
            ...
            query2 = `SELECT * FROM Divisions ORDER BY maxAge DESC;`
...
app.get('/seasonsdivisions', function(req, res){
    let query1;
    if(req.query.divisionID === undefined){
        query1 = "SELECT SeasonsDivisions.seasonDivisionID, Seasons.description AS 'Season',\
...
app.get('/teams', function(req, res){
    ...
    if(req.query.name === undefined){
        ...
        ' ', Adults.lastName) as 'headCoach' FROM Teams \
...
app.get("/adults", function(req, res) {
    let query1 = `SELECT * FROM Adults ORDER BY lastName, firstName;`;
...
app.post("/addAdult", function(req, res) {
    ...
    db.pool.query(query1, function(error, rows, fields){
        ...
        if (error) {
            ...
            // If there was no error, perform a query2
            query2 = `SELECT * FROM Adults ORDER BY lastName, firstName;`
...
app.get('/players', function(req, res){
    ...
    let query2;
    if(req.query.divisionID === undefined){
        query1 = "SELECT Players.playerID AS 'Select', Seasons.description AS 'RecentSeason', Divisions.description AS 'Division', Teams.name AS 'RecentTeam', \
...
app.delete("/deletePlayer", function(req,res,next){
    ...
    db.pool.query(query1, [playerID], function(error, rows, fields){
        ...
        // send status 204 to reload page
        res.sendStatus(204);
...
app.get('/volunteers', function(req, res){
    ...
    if(req.query.role === undefined){
        query1 = "SELECT volunteerID AS 'Select', Seasons.description AS 'Season', \
...
app.get("/games", function(req, res) {
    let query1 = `SELECT gameID, DATE_FORMAT(Games.dateTime, '%Y-%m-%d %r') as dateTime,\
     homeScore, awayScore, fieldNumber,\
      (SELECT Teams.name FROM Teams WHERE Teams.teamID= Games.homeTeamID)AS 'homeTeamID',\
       (SELECT Teams.name FROM Teams WHERE Teams.teamID= Games.awayTeamID) AS 'awayTeamID',\
        concat(Adults.firstName, ' ', Adults.lastName) AS 'refereeID' FROM Games\
     LEFT JOIN Volunteers ON Volunteers.volunteerID = Games.refereeID\
     LEFT JOIN Adults ON Adults.adultID = Volunteers.adultID
     ;`;
     
    let query2 = `SELECT Teams.teamID, Teams.name FROM Teams ORDER BY Teams.name`;
    let query3 = `SELECT Volunteers.volunteerID as 'volunteerID', concat(Adults.firstName,' ' , Adults.lastName) as 'referee' FROM Volunteers
                  INNER JOIN Adults ON Volunteers.adultID = Adults.adultID
                  WHERE Volunteers.role = 'Referee'
                  GROUP BY Adults.adultID\
                    ORDER BY Adults.lastName, Adults.firstName`
    ...
    db.pool.query(query1, function(error, rows, fields) {
        ...
        db.pool.query(query2, function(error, rows, fields){
            ...
            db.pool.query(query3, function(error, rows, fields){
                let refs = rows;
                return res.render('games', {data: games, teams: teams, refs: refs})
            })
        })    
    })
});
...
app.get('/orders', (req, res) => {
    ...
    ORDER BY SeasonsDivisions.seasonDivisionID`
        ...
        // If joining as a volunteer
            ...
            db.pool.query(query2, function(error, row, fields){
                if (error){
                    ...
                    let parseAdultID = parseInt(row[0].adultID);
                    let vTeamID = parseInt(data['teamID']);
                    if (isNaN(vTeamID)){
                        vTeamID = 'NULL';
                    }
                    query3 = `INSERT INTO Volunteers (role, details, adultID, teamID)
                    VALUES ('${data['volunteerRole']}', '${data['volunteerDetails']}', ${parseAdultID}, ${vTeamID})`;
                    db.pool.query(query3, function(error, rows, fields){
                        // If there was an error on the second query, send a 400
                        if (error){
                            console.log(error);
...
app.post('/insertOrderAjax', function(req, res){
    ...
    query0 = `SELECT seasonFee FROM Seasons
              WHERE description = 'Spring 2023'`
//    db.pool.query(query0, function(error, fee, fields){
//        if(error){
//            console.log(error);
//            res.sendStatus(400);
//        }
